...
/**
 * 重载运算符，用来生成节点的比较
 * 逻辑，該函數在“boost::heap::compare<CompareNodes>”获得使用
 */
struct CompareNodes {
...
Node3D* Algorithm::hybridAStar(Node3D& start, const Node3D& goal,
  ...
  // for reversing
  int dir =
      Constants::reverse ? 6 : 3;  //如果是后退，那么可能的前进方向为6；否则为3
  ...
  while (!O.empty()) {
    ...
    //    for (priorityQueue::ordered_iterator it = O.ordered_begin(); it !=
    //    O.ordered_end(); ++it) {
    //      succ = (*it);
    //      std::cout << "VAL"
    //                << " | C:" << succ->getC()
    ...
    // set index
    iPred = nPred->setIdx(
        width, height);  //获取该点在nodes3D的索引 (前缀i表示index, n表示node)
    iterations++;  //记录迭代次数
    ...
    // EXPANSION OF NODE
      ...
      // CONTINUE WITH SEARCH
        ...
        // SEARCH WITH DUBINS
        // SHOT//车子是在前进方向，优先考虑用Dubins去命中目标点
        ...
        for (int i = 0; i < dir; i++) {  //每个方向都搜索
          ...
          if (nSucc->isOnGrid(width, height) &&
            ...
            // as the predecessor 确定新扩展的节点不在close
            // list中，或者没有在之前遍历过
            if (!nodes3D[iSucc].isClosed() || iPred == iSucc) {
              ...
              // if successor not on open list or found a shorter way to the
              // cell 如果扩展节点不在OPEN
              // LIST中，或者找到了更短G值的路径，或者该节点索引与前一节点在同一网格中（用新点代替旧点）
              // 则进入更新新点的属性：cost-to-go，
              if (!nodes3D[iSucc].isOpen() || newG < nodes3D[iSucc].getG() ||
                  iPred == iSucc) {
                // calculate H value: 更新cost-to-go
                updateH(*nSucc, goal, nodes2D, dubinsLookup, width, height,
                        configurationSpace, visualization);

                // if the successor is in the same cell but the C value is
                // larger
                if (iPred == iSucc &&
                    nSucc->getC() > nPred->getC() + Constants::tieBreaker) {
                ...
                // 如果下一节点仍在相同的cell,
                // 但是代价值要小，则用当前successor替代前一个节点（这里仅更改指针，数据留在内存中）
                else if (iPred == iSucc &&
                         nSucc->getC() <=
                             nPred->getC() + Constants::tieBreaker) {
                  nSucc->setPred(
                      nPred
                          ->getPred());  //如果下一个点仍在相同的cell、并且cost变小，成功
                }
...
float aStar(Node2D& start, Node2D& goal, Node2D* nodes2D, int width, int height,
  ...
  while (!O.empty()) {
    ...
    // if there exists a pointer this node has already been expanded
    if (nodes2D[iPred]
            .isClosed()) {  //检查：如果已扩展，则从open set中移除，处理下一个
      // pop node from the open list and start with a fresh node
      O.pop();
      continue;
    }
    ...
    // EXPANSION OF NODE
      ...
      // CONTINUE WITH SEARCH
        ...
        for (int i = 0; i < Node2D::dir;
             i++) {  // A*算法是8个方向：4个正方向和4个45度的方向
          // create possible successor
...
//###################################################
// 计算到目标的启发值(cost)
// 这里的cost由三项组成：《Practical Search Techniques in Path Planning for
// Autonomous Driving》
// 1) "non-holonomic-without-obstacles" heuristic:（用于指导搜索向目标方向前进）
//    受运动学约束的无障碍启发式值。论文的计算建议为：
//    max(Reed-Shepp距离/Dubins距离, 欧氏距离) 表示
//    至于用Reed-Shepp距离还是Dubins距离取决于车辆是否可倒退
// 2) "holonomic-with-obstacles" heuristic：（用于发现U形转弯(U-shaped
// obstacles)/死路(dead-ends)）
//    （不受运动学约束的）有障约束启发式值(即：A*)
// 注1：
// 实际计算时，优先考虑运动学启发式值，A*作为可选项。至于是否启用欧氏距离和A*的启发式值，取决于计算
//      的精度和CPU性能（可作为调优手段之一）
// 注2： 实际计算与论文中的描述存在差异：
//      （1）实际计算的第一步用的启发式值为“Reed-Shepp距离/Dubins距离”，而论文为“max(Reed-Shepp距离/Dubins距离,
//      欧氏距离)” （2）实际计算的第二步用的启发式值为A*的启发式值 减去
//      “start与goal各自相对自身所在2D网格的偏移量(二维向量)的欧氏距离”
...
void updateH(Node3D& start, const Node3D& goal, Node2D* nodes2D,
  ...
  if (Constants::dubins) {
    ...
    //    // if the lookup table flag is set and the vehicle is in the lookup
    //    area if (Constants::dubinsLookup && uX < Constants::dubinsWidth - 1 &&
    //    uY < Constants::dubinsWidth - 1) {
    ...
    //      // mirror on x axis
    //      if (X >= 0 && Y <= 0) {
    //        h0 = (int)(helper::normalizeHeadingRad(M_PI_2 - t) /
    //        Constants::deltaHeadingRad); h1 =
    //        (int)(helper::normalizeHeadingRad(M_PI_2 - goal.getT()) /
    //        Constants::deltaHeadingRad);
    //      }
    //      // mirror on y axis
    //      else if (X <= 0 && Y >= 0) {
    //        h0 = (int)(helper::normalizeHeadingRad(M_PI_2 - t) /
    ...
    //      // mirror on xy axis
    //      else if (X <= 0 && Y <= 0) {
    //        h0 = (int)(helper::normalizeHeadingRad(M_PI - t) /
    //        Constants::deltaHeadingRad); h1 =
    //        (int)(helper::normalizeHeadingRad(M_PI - goal.getT()) /
    //        Constants::deltaHeadingRad);
    ...
    //      dubinsCost = dubinsLookup[uX * Constants::dubinsWidth *
    //      Constants::headings * Constants::headings
    //                                + uY *  Constants::headings *
    //                                Constants::headings
  ...
  if (Constants::twoD) {
    ...
    twoDCost = nodes2D[(int)start.getY() * width + (int)start.getX()].getG() -
               twoDoffset;
    // getG()返回A*的启发式代价，twoDoffset为
    // start与goal各自相对自身所在2D网格的偏移量的欧氏距离
  }

  // return the maximum of the heuristics, making the heuristic admissable
  start.setH(std::max(
      reedsSheppCost,
      std::max(dubinsCost, twoDCost)));  //将两个代价中的最大值作为启发式值
  //注：虽然这里有三个数值，但Dubins Cost的启用和Reeds-Shepp
  // Cost的启用是互斥的，所以实际上是计算两种cost而已
}
